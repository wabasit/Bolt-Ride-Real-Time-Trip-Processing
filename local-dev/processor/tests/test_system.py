...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('logs/trip_processor.log'),
...
class LocalDatabase:
    ...
    def __init__(self, db_path: str = "data/trip_data.db"):
...
def test_processor_api():
    """Test the processor API with sample events"""
    ...
    # Test trip_start event
    print("🚀 Testing trip_start event...")
    trip_start = {
        ...
        "vendor_id": 1,
        "pickup_datetime": datetime.utcnow().isoformat() + 'Z',
        "estimated_dropoff_datetime": datetime.utcnow().isoformat() + 'Z',
        "estimated_fare": 25.50,
        "timestamp": datetime.utcnow().isoformat() + 'Z',
        "metadata": {"source": "test_script"}
    }
    
    response = requests.post(f"{base_url}/events", json=trip_start)
    if response.status_code == 200:
        print("✅ Trip start event processed")
    else:
        print(f"❌ Trip start failed: {response.text}")
        return False
    ...
    # Test trip_end event
    print("🏁 Testing trip_end event...")
    trip_end = {
        "event_type": "trip_end",
        "schema_version": "2.0",
        "trip_id": "test_trip_001",
        "dropoff_location": {"lat": 5.56, "lon": -0.19},
        "dropoff_datetime": datetime.utcnow().isoformat() + 'Z',
        "final_fare": 28.75,
        "tip_amount": 3.00,
        "distance_km": 5.2,
        "duration_minutes": 15,
    ...
    response = requests.post(f"{base_url}/events", json=trip_end)
    if response.status_code == 200:
        print("✅ Trip end event processed")
    else:
        print(f"❌ Trip end failed: {response.text}")
        return False
    ...
    if response.status_code == 200:
        ...
        print(f"   Total events: {metrics['events']['total_events']}")
        print(f"   Trip starts: {metrics['events']['trip_starts']}")
        print(f"   Trip ends: {metrics['events']['trip_ends']}")
        print(f"   Completed trips: {metrics['trips']['completed_trips']}")
    else:
        print("❌ Could not retrieve metrics")
        return False
    
    # Check trip status
    print("🔍 Checking trip status...")
    response = requests.get(f"{base_url}/trip/test_trip_001")
    if response.status_code == 200:
        trip_data = response.json()
        print(f"✅ Trip status: {trip_data['status']}")
    else:
        print("❌ Could not retrieve trip status")
        return False
